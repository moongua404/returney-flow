...
public interface PromptRenderer {
  ...
  /**
   * systemTemplate이 정의된 경우 렌더링 결과를 반환하고, 없으면 null을 반환한다.
   * null은 systemTemplate이 미정의된 상태를 의미한다.
   */
  default String renderSystemPrompt(String action, Map<String, String> variables) {
    return null;
  }

  /**
   * userTemplate을 렌더링한다. renderSystemPrompt()가 non-null일 때만 유효하다.
   */
  default String renderUserPrompt(String action, Map<String, String> variables) {
    return render(action, variables);
  }
}
